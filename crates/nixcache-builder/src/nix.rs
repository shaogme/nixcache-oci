...
use std::{fmt, path::Path, process::Stdio, sync::Arc};
...
impl NixCli {
    ...
    pub async fn discover_outputs(
        &self,
        config: &BuildConfig,
    ) -> Result<Vec<BuildTarget>, BuilderError> {
        match config.mode {
            BuildMode::Flake => {
                let system = match &config.system {
                    Some(s) if !s.trim().is_empty() => s.trim().to_string(),
                    _ => self.current_system().await?,
                };
                info!(
        ...
        for store_path in paths {
            ...
            let task = tokio::spawn(async move {
                ...
                let path_info_out = Command::new("nix")
                    ...
                    .await
                    .map_err(|e| {
                        format!("Failed to run nix path-info for {}: {}", store_path, e)
                    })?;
                ...
                if let Some(refs_arr) = references {
                    for r_val in refs_arr {
                        if let Some(r_str) = r_val.as_str()
                            && let Some(bname) =
                                Path::new(r_str).file_name().and_then(|n| n.to_str())
                        {
...
pub async fn discover_outputs(config: &BuildConfig) -> Result<Vec<BuildTarget>, BuilderError> {
    NixCli.discover_outputs(config).await
}
...
pub async fn export_paths_directly(
    ...
    NixCli
        .export_paths_directly(paths, signing_key_file, cache_dir)
        .await
}
