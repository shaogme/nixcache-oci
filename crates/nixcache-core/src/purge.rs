use crate::{
    ...
    types::{CacheIndexData, IndexEntry, NarDigest, StoreHash, SystemArch},
};
        ...
        for root in roots {
            ...
            while let Some(curr) = q.pop_front() {
                if let Some(deps) = forward_graph.get(&curr) {
                    for dep in deps {
                        if purged_hashes.contains(dep) || !index.entries.contains_key(dep) {
                            is_sound = false;
                            break;
                        }
                ...
                } else if !index.entries.contains_key(&curr) {
                    is_sound = false;
                    break;
                }
                if !is_sound {
...
pub fn evaluate_cache_purge(
    ...
    let updated_gc_roots = prune_broken_gc_roots(index, &purged_hashes_set);
