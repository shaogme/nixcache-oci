...
impl CacheStore {
    ...
    /// 获取生产基线分片根索引 (L1 Memory -> L2 KV -> L3 GHCR，纯粹单原子 baseline_v8_{system})
    ...
    pub async fn get_status(&self, env: &Env) -> RemoteStatus {
        ...
        RemoteStatus {
            ...
            baseline_tag: self.config.baseline_tag.clone(),
            tier0_hot_entries: 0,
...
mod tests {
    ...
    use nixcache_core::{
        IndexEntry, NarDigest, NarInfoMeta, SCHEMA_VERSION_V8, ShardDataPayload,
    ...
    fn test_remote_status_serialization() {
        let status = RemoteStatus {
            ...
            baseline_tag: "cache-index-e2e".to_string(),
            tier0_hot_entries: 0,
            baseline_entries: 3,
