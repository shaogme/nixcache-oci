#!/usr/bin/env bash
# test-pipeline-receipt-single-writer.sh — End-to-end integration test for Schema v8 BuildReceipt Single-Writer Pipeline & 2-Tier Proxy
# Verifies:
...
echo "=== Starting NixCache Schema v8 BuildReceipt Single-Writer Pipeline Test ==="
...
assert idx['version'] == 8, f'Expected version 8, got {idx[\"version\"]}'
...
print('>>> Promoted cache-index verified (Schema v8, single-writer compacted 4 entries across shards).')
"
...
echo "=== ALL SCHEMA V8 RECEIPT & SINGLE-WRITER PIPELINE TESTS PASSED ==="
